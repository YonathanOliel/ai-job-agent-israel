...
describe('GreenhouseJobSource', () => {
  ...
  it('keeps only Israel-located tech roles and continues past a broken board', async () => {
    ...
    const jobs = await new GreenhouseJobSource(
      buildConfig('acme|Acme Corp,brokenco'),
      registry,
    ).fetchJobs();
  ...
  it('returns nothing when no companies are configured', async () => {
    const jobs = await new GreenhouseJobSource(buildConfig(''), registry).fetchJobs();
    expect(jobs).toEqual([]);
  });
  ...
  it('prettifies the display name when only a token is given', async () => {
    ...
    const jobs = await new GreenhouseJobSource(buildConfig('big-data-co'), registry).fetchJobs();
